    ...
    FlowGenerator.prototype.renderMainMethodFields = function (operation, fields) {
        ...
        var methods = Object.keys(fields)
            ...
            return "    " + field.name + "(args" + (hasArgs ? '' : '?') + ": {" + (hasArgs ? ' ' : '') + field.args
                ...
                .join(', ') + (field.args.length > 0 ? ' ' : '') + "}, info?: GraphQLResolveInfo | string, options?: Options): " + _this.getPayloadType(operation, "" + _this.renderFieldType(field.type) + (!graphql_1.isNonNullType(field.type) ? ' | null' : '')) + "; ";
        })
            .join('\n');
        return "{\n" + methods + "\n  }";
    };

...
export default () => {
  ...
  function Generator(seed, size) {
    ...
    this.setLightingData = (setLighting, setSmoothLighting, get, coordx, coordy, coordz, changedBlocks) => {
      const offsets = [coordx * size - 1, coordy * size - 1, coordz * size - 1]
        ...
        for (let z = 1; z < size + 1; z++)
          for (let y = 1; y < size + 1; y++) {
            if (get(x, z, y) !== 0) {
              ...
              const lighting = generator.getBlockLighting(tempx, tempy, tempz, get, offsets, changedBlocks);
              for (let l = 0; l < 6; l++) {
                setLighting(x - 1, z - 1, y - 1, l, lighting[l]);
              }

              const smoothLighting = generator.getBlockSmoothLighting(x, y, z, get)
              for (let l = 0; l < 6; l++) {
                if (smoothLighting[l]) {
                  for (let m = 0; m < 3; m++)
                    for (let n = 0; n < 3; n++) {
                      setSmoothLighting(x - 1, z - 1, y - 1, l, m, n, smoothLighting[l][m][n])
                    }
                }
              }
            }
    ...
    this.getBlockLighting = (x, y, z, get, offsets, changedBlocks) => {
      ...
      for (let i = 0; i < surroundings.length; i++) {
        ...
        const value = this.getLoadedBlocks(block.x, block.y, block.z, get, offsets, changedBlocks)
        if (value === 0) {
          ...
          while (queue.length > 0) {
            ...
            for (let n = 1; n < surroundings.length - 1; n++) {
              ...
              let endValue = this.getLoadedBlocks(newNode.x, yValue, newNode.z, get, offsets, changedBlocks)

              while (startValue === 0 && endValue !== 0) {
                yValue += 1;
                startValue = this.getLoadedBlocks(q.x, yValue, q.z, get, offsets, changedBlocks)
                endValue = this.getLoadedBlocks(newNode.x, yValue, newNode.z, get, offsets, changedBlocks)
              }
    ...
    this.getLoadedBlocks = (x, y, z, get, offsets, changedBlocks) => {
      ...
      if (this.checkWithinChunk(relativeCoords[0], relativeCoords[1], relativeCoords[2])) {
    ...
    this.getRelativeCoords = (x, y, z, offsets) => ([x - offsets[0], y - offsets[1], z - offsets[2]])

    this.getAbsoluteCoords = (x, y, z, offsets) => ([x + offsets[0], y + offsets[1], z + offsets[2]])

    this.checkWithinChunk = (x, y, z) => (x >= 0 && x < size + 2 && y >= 0 && y < size + 2 && z >= 0 && z < size + 2)
  }
}

...
pub fn get_chunk_name(coords: &Coords2<i32>, concat: &str) -> String {
    format!("{}{}{}", coords.0, concat, coords.1)
}

pub fn get_voxel_name(coords: &Coords3<i32>, concat: &str) -> String {
    format!("{}{}{}{}{}", coords.0, concat, coords.1, concat, coords.2)
}

pub fn parse_chunk_name(name: &String, concat: &str) -> Coords2<i32> {
    let vec = name.split(concat).collect::<Vec<&str>>();
    return Coords2(vec[0].parse().unwrap(), vec[1].parse().unwrap());
}

pub fn parse_voxel_name(name: &String, concat: &str) -> Coords3<i32> {
    let vec = name.split(concat).collect::<Vec<&str>>();
    return Coords3(
        vec[0].parse().unwrap(),
        vec[1].parse().unwrap(),
...
pub fn map_voxel_to_chunk(voxel_pos: &Coords3<i32>, chunk_size: i32) -> Coords2<i32> {
...
pub fn map_voxel_to_chunk_local(voxel_pos: &Coords3<i32>, chunk_size: i32) -> Coords3<i32> {

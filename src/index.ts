...
const _pickPos = vec3.create() as Vector;
...
export interface IEngineOptions
  extends Partial<ICameraOptions>,
    Partial<IEntitiesOptions>,
    Partial<IContainerOptions>,
    Partial<IPhysicsOptions>,
    Partial<IInputOptions>,
    Partial<IWorldOptions>,
    Partial<IRegistryOptions>,
    Partial<IRenderingOptions> {
  /**
   * @default false
   */
  debug: boolean;
...
class Engine extends EventEmitter {
  constructor(options: Partial<IEngineOptions>) {
    ...
    if (!optionsWithDefaults.silent) {
      const debugstr = optionsWithDefaults.debug ? ' (debug)' : '';
    ...
    const body = this.ents.getPhysicsBody(this.playerEntity);
  ...
  render = (framePart: number) => {
    ...
    const dt = t - (this._lastRenderTime || t - 16);
  ...
  localToGlobal = (local: number[], global: number[], globalPrecise?: number[]) => {
    ...
    if (globalPrecise) {
      for (let i = 0; i < 3; i++) {
        const floored = Math.floor(local[i]);
...
function updateBlockTargets(noa: Engine) {
  ...
  const result = noa._localPick(null, null, null, blockIdFn);
  if (result) {

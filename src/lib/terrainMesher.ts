...
function buildPaddedVoxelArray(chunk: Chunk) {
  ...
  const cs = src.shape[0];
  let tgt = cachedPadded;
  ...
  const pos = _vecs[1];
  const size = _vecs[2];
  const tgtPos = _vecs[3];
  const posValues = _vecs[4];
  const sizeValues = _vecs[5];
  const tgtPosValues = _vecs[6];
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      for (let k = 0; k < 3; k++) {
        ...
        const nab = chunk._neighbors.get(i - 1, j - 1, k - 1);
        const nsrc = nab ? nab.voxels : null;
        copyNdarrayContents(
          nsrc,
          tgt,
          pos as [number, number, number],
          size as [number, number, number],
          tgtPos as [number, number, number],
        );
      }
    }
  }
  ...
  build(chunk: Chunk, meshDataList: Submesh[], ignoreMaterials: boolean) {
    ...
    for (let i = 0; i < meshDataList.length; i++) {
      ...
      if (ignoreMaterials) {
        ...
        mdat.mergeable = !url && matData.alpha === 1 && !matData.renderMat;
      }
      if (mdat.mergeable) {
        numMergeable++;
      }
...
class GreedyMesher {
  ...
  mesh = (
    ...
    const subMeshes: { [key: string]: Submesh } = {};

    // precalc how to apply AO packing in first masking function
    const skipReverseAO = revAoVal === aoValues[0];
  ...
  constructMeshMasks = (i: any, d: any, arrT: any, getMaterial: any) => {
    ...
    const mask = this.maskCache;
    const aomask = this.aomaskCache;
    ...
    for (let k = 0; k < len; ++k) {
      ...
      for (let j = 0; j < len; j++, n++, d0 += jstride) {
        ...
        const faceDir = this.getFaceDir(id0, id1, getMaterial, materialDir);
        if (faceDir) {
          ...
          if (this.aoPackFunction) {
            ...
            aomask[n] =
              faceDir > 0 ? this.aoPackFunction(arrT, i, i - 1, j, k) : this.aoPackFunction(arrT, i - 1, i, j, k);
          }
        }
      }
    }
  ...
  getFaceDir = (
    id0: number,
    id1: number,
    getMaterial: (id: number, materialDir: number) => number,
    materialDir: number,
  ) => {
    // no face if both blocks are opaque
    const op0 = this.opacityLookup[id0];
    const op1 = this.opacityLookup[id1];
  ...
  constructMeshDataFromMasks = (
    ...
    const maskCompareFcn = doAO ? this.maskCompare : this.maskCompare_noAO;
    const meshColorFcn = doAO ? this.pushMeshColors : this.pushMeshColors_noAO;
    ...
    for (let k = 0; k < len2; ++k) {
      ...
      for (let j = 0; j < len1; j += w, n += w) {
        ...
        const colors = mesh.colors! as Color4;
  ...
  pushMeshColors = (colors: number[], c: number[], ao: any, aoValues: any, revAoVal: any) => {
    const ao00 = this.unpackAOMask(ao, 0, 0);
    const ao10 = this.unpackAOMask(ao, 1, 0);
    const ao11 = this.unpackAOMask(ao, 1, 1);
    const ao01 = this.unpackAOMask(ao, 0, 1);
  ...
  packAOMaskNoReverse = (data: any, ipos: any, ineg: any, j: any, k: any) => {
    ...
    if (this.solidLookup[data.get(ipos, j + 1, k)]) {
      ++a10;
      ++a11;
    }
    if (this.solidLookup[data.get(ipos, j - 1, k)]) {
    ...
    if (this.solidLookup[data.get(ipos, j, k + 1)]) {
      ++a01;
      ++a11;
    }
    if (this.solidLookup[data.get(ipos, j, k - 1)]) {
    ...
    const facingSolid = this.solidLookup[data.get(ipos, j, k)];
    if (facingSolid) {
      ...
      if (a11 === 1 && this.solidLookup[data.get(ipos, j + 1, k + 1)]) {
        a11 = 2;
      }
      if (a01 === 1 && this.solidLookup[data.get(ipos, j - 1, k + 1)]) {
        a01 = 2;
      }
      if (a10 === 1 && this.solidLookup[data.get(ipos, j + 1, k - 1)]) {
        a10 = 2;
      }
      if (a00 === 1 && this.solidLookup[data.get(ipos, j - 1, k - 1)]) {
        a00 = 2;
      }
    }
    ...
    if (facingSolid) {
      ...
      if (a11 === 1) {
        if (this.solidLookup[data.get(ipos, j + 1, k + 1)]) {
          ...
          !this.solidLookup[data.get(ineg, j, k + 1)] ||
...
var profile_hook = PROFILE_EVERY ? makeProfileHook(PROFILE_EVERY, 'Terrain meshing') : () => {};

...
export class Registry {
  ...
  registerBlock = (id: number, options: Partial<IBlockOptions>) => {
    ...
    const hasHandler =
      ...
      optionsWithDefaults.onUnload ||
      optionsWithDefaults.onSet ||
      optionsWithDefaults.onUnset ||
      optionsWithDefaults.onCustomMeshCreate;
  ...
  registerMaterial = (
    name: string,
    color: Color3 | Color4 = [1, 1, 1],
    textureURL: string | undefined = undefined,
    texHasAlpha = false,
    renderMaterial: Material | null = null,
  ) => {
  ...
  getMaterialId = (name: string, lazyInit = false) => {
    if (!name) {
      return 0;
    }

    let id = this.matIDs[name];

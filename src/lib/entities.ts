...
export class Entities extends EntComp {
  ...
  setEntitySize(id: number, xs: any, ys: any, zs: any) {
    ...
    const getPos = this.getStateAccessor<IPosData>(this.names.position);
  ...
  isTerrainBlocked(x: number, y: number, z: number) {
    ...
    const blockExt = [xlocal + 0.001, ylocal + 0.001, zlocal + 0.001, xlocal + 0.999, ylocal + 0.999, zlocal + 0.999];

...
export class Chunk<UserDataType = any> {
  constructor(noa: Engine, id: string, i: number, j: number, k: number, size: number, dataArray: ndarray<any>) {
    ...
    const narr = Array.from(Array(27)).map(() => null);
  ...
  set = (x: number, y: number, z: number, newID: number) => {
    const oldID = this.voxels.get(x, y, z);
    const oldIDnum = oldID;
    if (newID === oldIDnum) {
      return;
    }
  ...
  updateMeshes = () => {
    ...
    if (this._terrainDirty) {
      ...
      if (mesh && mesh.getIndices()!.length > 0) {
        const pos: [number, number, number] = [this.x, this.y, this.z];
    ...
    if (this._objectsDirty) {
      ...
      const meshes = buildObjectMeshes(this);
      const pos2: [number, number, number] = [this.x, this.y, this.z];
  ...
  callBlockHandler = (
    blockID: number,
    type: 'onUnset' | 'onSet' | 'onLoad' | 'onUnload',
    x: number,
    y: number,
    z: number,
  ) => {
...
let terrainMesher: undefined | TerrainMesher;

// Registry lookup references shared by all chunks
let solidLookup: any[];
let opaqueLookup: any[];
let objectLookup: any[];
let blockHandlerLookup: any[];

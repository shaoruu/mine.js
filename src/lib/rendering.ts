...
export type noaMesh = BabylonMesh & {
...
const hlpos: [number, number, number] = [] as any;
...
export class Rendering {
  constructor(noa: Engine, options: Partial<IRenderingOptions>, canvas: HTMLCanvasElement | WebGLRenderingContext) {
    ...
    function arrToColor3(a: Color3) {
      return new Color3Babylon(a[0], a[1], a[2]);
    }
    function arrToColor4(a: Color4) {
      return new Color4Babylon(a[0], a[1], a[2], a[3]);
    }
  ...
  getHighlightMesh = () => {
    ...
    if (!mesh) {
      ...
      const hlm = this.makeStandardMaterial('highlightMat');
      ...
      // outline
      const s = 0.5;
      const lines = BabylonMesh.CreateLines(
        'hightlightLines',
        [
          new Vector3(s, s, 0),
          new Vector3(s, -s, 0),
   ...
   */
  addMeshToScene = (mesh: noaMesh, isStatic = false, pos?: [number, number, number], _containingChunk?: any) => {
  ...
  prepareChunkForRendering = (chunk: Chunk) => {
    const cs = chunk.size;
    const loc: [number, number, number] = [] as any;
    this.noa.globalToLocal([chunk.x, chunk.y, chunk.z], null, loc);
    const min = new Vector3(loc[0], loc[1], loc[2]);
    const max = new Vector3(loc[0] + cs, loc[1] + cs, loc[2] + cs);
  ...
  updateCameraForRender = () => {
    ...
    const tgtLoc = cam._localGetTargetPosition();
  ...
  checkCameraEffect = (id: number) => {
    ...
    if (id === 0) {
      ...
      const matId = this.noa.registry.getBlockFaceMaterial(id, 0);
      if (matId) {
        const matData = this.noa.registry.getMaterialData(matId);
        const col = matData.color;
        const alpha = matData.alpha;
        if (col && alpha && alpha < 1) {
          (this._camScreenMat as any).diffuseColor
            .set(
              0,
              0,
              0,
            )(this._camScreenMat as any)
            .ambientColor.set(col[0], col[1], col[2]);

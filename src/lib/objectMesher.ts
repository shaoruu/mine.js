...
export function buildObjectMeshes(chunk: Chunk) {
  ...
  for (const key in chunk._objectBlocks) {
    ...
    const blockDat: ObjMeshDat = chunk._objectBlocks[key];
...
export function buildSPSforMaterialIndex(chunk: Chunk, scene: any, meshHash: any, x0: number, y0: number, z0: number) {
  ...
  // base sps
  const sps = new SolidParticleSystem('object_sps_' + chunk.id, scene, {
    updatable: false,
  });
  ...
  for (const blockID in meshHash) {
    const mesh = objectMeshLookup[blockID as any];
    var blockArr = meshHash[blockID];
    const count = blockArr.length;

    var handlerFn: blockHandler;
    const handlers = blockHandlerLookup[blockID as any];
    if (handlers) {
      handlerFn = handlers.onCustomMeshCreate!;
    }

    const setShape = function (particle: any, partIndex: number, shapeIndex: number) {

    ...
    switch (cmd) {
      ...
      case 'GET_CHUNK': {
        ...
        if (blocks.find(ele => ele)) {
          ...
          const combined = combineMeshes(
            meshQuads(calcQuads(blocks, lighting, smoothLighting, dims), coordx, coordy, coordz)
          )
      ...
      case 'UPDATE_BLOCK': {
        ...
        if (data.find(ele => ele)) {
          ...
          self.generator.setLightingData(lighting, smoothLighting, data, coordx, coordy, coordz)

          const combined = combineMeshes(
            meshQuads(calcQuads(data, lighting, smoothLighting, dims), coordx, coordy, coordz)
          )

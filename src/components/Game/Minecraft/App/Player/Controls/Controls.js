...
class Controls extends Stateful {
  constructor(player, status, world, chat, camera, container, blocker, button, initPos, initDirs) {
    super()
    ...
    if (typeof this.mouseKey === 'number') {
      switch (this.mouseKey) {
        case 0: {
          ...
          } else if (this.status.isSurvival && !this.breakBlockCountdown) this._startBreakingBlock()
          break
        }
        case 2: {
          // Right Key
  ...
  _registerKeys = () => {
    ...
    this.keyboard.registerKey(27, 'chat', this.chat.disable, this._unblockGame, undefined, {
      repeat: false
    })
